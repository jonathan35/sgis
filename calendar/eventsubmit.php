...
if ($_SESSION['validation']=='YES') {//if (auth()) {
	switch ($_GET['flag']) {
		case "add" :
			submitEventData();
			break;
		...
		case "delete":
			...
			if (!(empty($id) && empty($month) && empty($year)))
				...
				echo "here1";//$lang['accesswarning'];
			break;
		default:
			echo "here2";//$lang['accesswarning'];
	}
} else {
	echo "here3";//echo $lang['accessdenied'];
}
...
function submitEventData ($id="")
{
	...
	if ($id) {
		...
		$sql .= "start_time='$starttime', end_time='$endtime', title='$title', text='$text', project_id='$project' ";
		$sql .= "WHERE id=$id";
		$result = $lang['updated'];
	} else {
