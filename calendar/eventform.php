...
function displayEditForm($mode, $uid, $id="")
{
	...
	if ($mode == "Add") {
		...
		$sql = "SELECT uid, y, m, d, start_time, end_time, title, text, project_id ";
		...
		if (!empty($row)) {
			...
			$d 			= $row["d"];
			$y 			= $row["y"];
			$shit = stripslashes($row["project_id"]);
		}
...
?>
	...
	<br><img src="images/clear.gif" width="1" height="5"><br>
		...
		<input type="hidden" name="uid" value="<?php echo $uid?>">
			<tr>
				<td nowrap valign="top" align="right"><span class="form_labels"></span></td>
...
<!-- Extra add in --><!-- 
			<tr>
				...
				<span class="form_labels">Project</span></td>
				<td>
				    <?
                    	//$project_query=mysqli_query($conn, "select * from prj where status=1");
						//$project=mysqli_fetch_assoc($project_query);
					?>
                    <select name="project" id="project">
				      <option value="#">Select Project</option>
                      <?php //do{?>
                      <option value="<?php //echo $project['id']?>" <?php //if($project['id']==$row['project_id']){?>selected<?php //}?>><?php //echo $project['prjt_name']?> </option>
                      <?php //}while($project=mysqli_fetch_assoc($project_query));?>
			        </select>
...
<!-- End at here -->            
			...
			<tr>
				<td nowrap valign="top" align="right"><span class="form_labels"><?php echo $lang['starttime'] ?></span></td>
			...
			<tr>
				<td nowrap valign="top" align="right"><span class="form_labels"><?php echo $lang['endtime'] ?></span></td>
