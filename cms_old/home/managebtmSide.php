...
session_start();
	...
	if($_GET['wp']==1){
		$ssss = mysqli_query($conn, "select description from home_section where id=5 ");
		$asdss= mysqli_fetch_assoc($ssss);
		$myFile = $asdss['description'];
			if(file_exists($myFile)){
			@unlink($myFile);
			}
			mysqli_query($conn, "update home_section set description='' where id=5");
	}
	...
	if($_POST['submit']=='Submit')
	{
		if($_FILES['xxx']!='')
		{
			...
			{    
				...
				{mysqli_query($conn, "update home_section set description='".$file."', date='".date("Y-m-d")."' where id=5");} 
			}
		}					
	...
	if(mysqli_query($conn, "update home_section set description='$description', date='".date("Y-m-d")."' where id=4"))
...
<table width="100%" align="left" cellpadding="2"  cellspacing="3">
	<form name="form1" method="post" action="managebtmSide.php" enctype="multipart/form-data">
	<tr>
	  <td colspan="2" height="32" class="main_title" align="center" bgcolor="#CCCCCC" valign="middle">EDIT BOTTOM</td>
	</tr>
	<tr>
	...
	<?php echo $save;?>
	  ...
	  <tr>
<th align="right" bgcolor="#EFEFEF" class="main_title" valign="top">Bottom Background <br>
  <span class="red02">Recommended images size 975 x 180 pixels</span></th>
          <td width="80%" class="content"> Photo 1&nbsp;&nbsp;&nbsp;<?php if($selected2['description']!='' && file_exists("../../".$selected2['description'])){?><a href="managebtmSide.php?<?php echo $_SERVER['QUERY_STRING']?>&wp=1"><img src="../images/b_drop.png" width="16" height="15" border="0"></a><?php }?><br>
				<?php if($selected2['description']!=''&& file_exists("../../".$selected2['description'])){$photo1="../../".$selected2['description'];}else{$photo1='../images/no_photo.gif';}?>
                  <img name="thisImage1" id="thisImage1" src="<?php echo $photo1?>" width="243" height="45"><br>
                  <input name="xxx" type="file" class="content" onChange="javascript:findItem('thisImage1').src = findItem('photo1').value;">
            </td>
</tr>
