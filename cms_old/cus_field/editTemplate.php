...
session_start();
	...
	if($_GET['wp']!=''){
		$field_delete = base64_decode($_GET['wp']); 
		$ssss = mysqli_query($conn, "select $field_delete from custom_field_section where id='".$abc."' ");
		$asdss= mysqli_fetch_assoc($ssss);
		$myFile = "../../".$asdss[$field_delete];
			if(file_exists($myFile)){
			@unlink($myFile);
			}
			mysqli_query($conn, "update custom_field_section set $field_delete='' where id='".$abc."'");
	}	
	...
	if($_FILES[$field_name[$h]]!=''&&$HTTP_POST_FILES[$field_name[$h]]['tmp_name']!='')//TO CHECH IS THE FIELD IS FILE BROWSE TYPE OR NOT--
			...
			if($Active_Post_File[$h]!=''&&$_POST[$field_name[$h]]==''){
				$qry[$h] = "$field_name[$h]=$field_name[$h]";
			}elseif($_POST[$field_name[$h]]!=''){ 
				$qry[$h] = "$field_name[$h]='".$_POST[$field_name[$h]]."'";
			}else{
				$qry[$h] = "$field_name[$h]=''";
			}
	...
	//echo $qry[1].$qry[2].$qry[3].$qry[4].$qry[5].$qry[6].$qry[7].$qry[8].$qry[9].$qry[10].$qry[11].$qry[12].$qry[13].$qry[14].$qry[15].$qry[16].$qry[17].$qry[18].$qry[19].$qry[20].$qry[21].$qry[22].$qry[23].$qry[24].$qry[25].$qry[26].$qry[27].$qry[28].$qry[29].$qry[30];
	
	
	if(mysqli_query($conn, "update custom_field_section set $qry[1] $qry[2] $qry[3] $qry[4] $qry[5] $qry[6] $qry[7] $qry[8] $qry[9] $qry[10] $qry[11] $qry[12] $qry[13] $qry[14] $qry[15] $qry[16] $qry[17] $qry[18] $qry[19] $qry[20] $qry[21] $qry[22] $qry[23] $qry[24] $qry[25] $qry[26] $qry[27] $qry[28] $qry[29] $qry[30] where id='".$abc."'"))
...
$selected_query=mysqli_query($conn, "select * from custom_field_section where id='$abc'");
...
<table width="100%" align="left" cellpadding="2"  cellspacing="3">
	...
	<form name="form1" method="post" action="editTemplate.php?m=<?php echo $_GET['m']?>&id=<?php echo $_GET['id']?>" enctype="multipart/form-data">
	...
	<?php do{?>
     ...
     <tr valign="top">
		...
		<?php if($field_set['field_type']=='file'||$field_set['field_type']=='file02'){?> 
			<?php if($selected[$field_set['table_field']]!=''&& file_exists("../../".$selected[$field_set['table_field']])){?>
				<a href="editTemplate.php?<?php echo $_SERVER['QUERY_STRING']?>&wp=<?php echo base64_encode($field_set['table_field'])?>"><img src="../images/b_drop.png" width="16" height="15" border="0"></a>
			 <?php if($field_set['field_type']=='file02'){?>&nbsp;
                  <a href="<?php echo "../../".$selected[$field_set['table_field']];?>" target="_blank">view</a><br>               
             <?php }else echo "<br>";?> 
			<?php $photo1="../../".$selected[$field_set['table_field']];}
				else{
					$photo1='../images/no_photo.gif';
					}?>
                    <?php if($field_set['field_type']=='file'){?>
                  <img name="thisImage1" id="thisImage1" src="<?php echo $photo1?>" width="100" height="75"><br>
				 
				  <?php }?>
            <input name="<?php echo $field_set['table_field']?>" type="file" class="content" onChange="javascript:findItem('thisImage1').src = findItem('photo1').value;"><?php if($field_set['field_type']=='file'){?><input type="hidden" name="<?php echo "file".$field_set['table_field']?>" value="<?php echo $field_set['table_field']?>"><?php }?>
		 ...
		 <?php }elseif($field_set['field_type']=='text'||$field_set['field_type']=='position'||$field_set['field_type']=='price'){?> 
           ...
           <?php if($field_set['field_type']=='price'){?><?php echo "<span class=\"red02\">".$field_set['price_currency']."</span>"?><?php }?>
            <input name="<?php echo $field_set['table_field']?>" type="text"<?php if($field_set['field_limit']!=''){?> maxlength="<?php echo $field_set['field_limit']?>"<?php }?> value="<?php echo $selected[$field_set['table_field']]?>">
         ...
         <?php }elseif($field_set['field_type']=='textarea'){?> 
            <textarea name="<?php echo $field_set['table_field']?>" rows="5" cols="50"<?php if($field_set['field_limit']!=''){?> maxlength="<?php echo $field_set['field_limit']?>"<?php }?>><?php echo $selected['description']?><?php echo $selected[$field_set['table_field']]?></textarea>
         <?php }elseif($field_set['field_type']=='tiny'){?> 
            <textarea name="<?php echo $field_set['table_field']?>" class="mceEditor" rows="15" cols="50"<?php if($field_set['field_limit']!=''){?> maxlength="<?php echo $field_set['field_limit']?>"<?php }?>><?php echo $selected[$field_set['table_field']]?></textarea>
        <?php }?> 
        </td>        
	</tr>	
