...
if($_POST['trash']=='Trash'){	
	...
	if(!empty($_POST['productIdList'])){
		foreach($items_trash_array as $items_trash){
			$trash_data="update mydf_product set status='-1' where id='".$items_trash."'";
			mysqli_select_db($conn, $database_pamconnection);
			$dataResult1 = mysqli_query($conn, $trash_data) or die(mysqli_error());			
		}
	}
}
...
if($_POST['delete']=='Delete'){	
	...
	if(!empty($_POST['productIdList'])){
		foreach($items_delete_array as $items_delete){
			...
			$file_check_query = mysqli_query($conn, "SELECT * FROM mydf_product WHERE id='".$items_delete."' and status='-1'");
			$file_check = mysqli_fetch_assoc($file_check_query);
			
			if(!empty($file_check['attachment_file'])){
				unlink('../../'.$file_check['attachment_file']);
			}
			if(!empty($file_check['description'])){
							
				$file_count = substr_count($file_check['description'], 'img');
								
				for ($i=0; $i < $file_count; $i++){
				  $fileCheck_query = mysqli_query($conn, "SELECT * FROM mydf_product WHERE id='".$items_delete."' and status='-1'");
				  $fileCheck = mysqli_fetch_assoc($fileCheck_query);
				  
				  $from = strpos($fileCheck['description'], 'photo');
				  $to = strpos($fileCheck['description'], '" alt');
				  $count_to = $to - $from;
				  
				  $imgFile = substr($fileCheck['description'], $from, $count_to);
				  unlink('../../'.$imgFile);
				  $replace = substr($fileCheck['description'], $from, 30);
				  
				  $imgReplace = str_replace($replace, "replace-- ", $fileCheck['description']);
				  
				  mysqli_query($conn, "UPDATE mydf_product SET description='".$imgReplace."' where id='".$items_delete."' ");
				}
			}
			
			mysqli_query($conn, "DELETE FROM mydf_product WHERE id='".$items_delete."' and status='-1'");
		}
	}
}
...
if($_POST['restore']=='Restore'){	
	...
	if(!empty($_POST['productIdList']))
	{
		...
		{
			$restore_data="update mydf_product set status='0' where id='".$items_restore."'";
			mysqli_select_db($conn, $database_pamconnection);
			$dataResult1 = mysqli_query($conn, $restore_data) or die(mysqli_error());			
		}
	}
}
...
if($_GET['tab']=="approvedondisplay"||$_GET['tab']==""){
	...
	$trash_query=mysqli_query($conn, "SELECT * FROM mydf_product WHERE status=-1 $get_mcat_query $get_scat_query");
                ...
                <div id="tab">
                  <ul>
                    ...
                    <li class=""><a href="manageProduct.php?tab=approvednotondisplay&main=<?php echo $_GET['main']?>&sub=<?php echo $_GET['sub']?>" title="Not Display Product Item Online" class="content">Not On Display (
                     ...
                     <li class=""><a href="manageProduct.php?tab=trash&main=<?php echo $_GET['main']?>&sub=<?php echo $_GET['sub']?>" title="Trash Product Item Online" class="content">Trash (
                                <?php if ($total_testimonial_trash!=0){echo $total_testimonial_trash;}else{?>
                            0
                            <?php }?>
                            )</a></li>
                  </ul>
...
&nbsp;&nbsp; 
<input type="submit" name="trash" value="Trash" onClick="return deleteProduct1();" title="Delete selected item(s)">
                </span>
                </div>
                <table border="0" cellpadding="4" cellspacing="1" width="100%">
                  ...
                  <tr bgcolor="<?php echo $bgcolor;?>">
                    <th width="9%" class="content" bgcolor="<?php echo $bgcolor;?>">
					  ...
					  ?>
                      <br> <?php echo "result.php?id=".base64_encode($row_Rs1['id']);?>
                      </span>
                      </td>
                     <td class="content">
...
<input type="submit" name="update" value="Save & Sort" onClick="return saveProduct();" title="Save & Sort item(s) position" style="width:90px; color:#333;">
 &nbsp;&nbsp;
<input type="submit" name="trash" value="Trash" onClick="return deleteProduct2();" title="Delete selected item(s)" />
...
?>
   ...
   <?php }while($rotcat = mysqli_fetch_assoc($root_Rs1));?>
              ...
              <!--Not On Display End-->
              <?php }elseif($_GET['tab']=="trash"){ ?>
              <!--Trash-->
              <form name="ProductListForm2" method="post" action="manageProduct.php?tab=trash&pageNum_Rs1=<?php echo $_GET['pageNum_Rs1'];?>&main=<?php echo $_GET['main']?>&sub=<?php echo $_GET['sub']?>">
			  <script>
                ...
                <div id="tab">
                  <ul>
                    ...
                    <li class=""><a href="manageProduct.php?tab=approvednotondisplay&main=<?php echo $_GET['main']?>&sub=<?php echo $_GET['sub']?>" title="Not Display Product Item Online" class="content">Not On Display (
                     ...
                     <li class="current"><a href="manageProduct.php?tab=trash&main=<?php echo $_GET['main']?>&sub=<?php echo $_GET['sub']?>" title="Trash Product Item Online" class="content">Trash (
                                <?php if ($total_testimonial_trash!=0){echo $total_testimonial_trash;}else{?>
                            0
                            <?php }?>
                            )</a></li>
...
<tr><td>
                ...
                <div class="content"> 
                Select  <input type="checkbox" name="checkbox" value="checkbox" checked disabled> 
				to: <input type="submit" name="restore" value="Restore" onClick="return restoreProduct();" title="Restore selected item(s)" />
                &nbsp;&nbsp;
