...
/*
Note
Actually is no point link uploaded file to root category, it is useless. Link to root cat feature just to make it nice to look only.
Because at front end retreiver, page will detect the pre-design page through the root category in murum_section table. not through the category field in file_upload02 table.
But it is usefull when link the uploaded file to category level 1 and level 2.
*/
		...
		if(!empty($_POST['productIdListUpdate']))
		{
			...
			{	 
				...
				if($data_check!=''){
					...
					mysqli_query($conn, "update file_upload02 set category='".$mcat_name."' where file_id='".$items_update."'");
					$data_final="";
					
				}else{
					$mcat_name = "";
					mysqli_query($conn, "update file_upload02 set category='' where file_id='".$items_update."'");
				}
	...
	$temp_query_Recordset1 = "SELECT * FROM file_upload02 WHERE status=1";					 
	...
	do
	{
		...
		{	
			...
			$insertSQL1 = "UPDATE file_upload02 SET 
	...
	$temp_query_Recordset2 = "SELECT * FROM file_upload02 WHERE status=0";					 
...
if($_POST['delete']=='Delete'){	
	...
	if(!empty($_POST['productIdList']))
	{
		...
		{
			$del_data="DELETE FROM file_upload02 WHERE file_id='$items_delete'";
			mysqli_select_db($conn, $database_pamconnection);
...
if($_GET['tab']=="approvedondisplay"||$_GET['tab']=="")
{
	if(!empty($_GET['alphabet'])) {
	 ...
	 $query="SELECT * FROM file_upload02 WHERE status=1 AND name like '$colname1_Rs1%' $get_mcat_query";
	 }
	 else
		$query="SELECT * FROM file_upload02 WHERE status=1 $get_mcat_query ORDER BY date DESC";
}
else if($_GET['tab']=="approvednotondisplay")
{
	if(!empty($_GET['alphabet'])) {
	 $colname1_Rs1=(get_magic_quotes_gpc()) ? $_GET['alphabet'] : addslashes($_GET['alphabet']);
	 $query="SELECT * FROM file_upload02 WHERE status=0 AND name like '$colname1_Rs1%' $get_mcat_query";
	 }
	 else	
		$query="SELECT * FROM file_upload02 WHERE status=0 $get_mcat_query ORDER BY date DESC";
}
	...
	$query2=mysqli_query($conn, "SELECT * FROM file_upload02 WHERE status=0 $get_mcat_query");
	$total_testimonial_not_on_display = mysqli_num_rows($query2);
	$query=mysqli_query($conn, "SELECT * FROM file_upload02 WHERE status=1 $get_mcat_query");
	$total_testimonial_on_display = mysqli_num_rows($query);

if($_POST['submit']=='update display')
	{
		if ($_POST['type']!='')
			mysqli_query($conn, "update file_upload02 set type='".$_POST['type']."' ");			
	}
	$query=mysqli_query($conn, "select * from file_upload02");
...
<table width="799" border="0" align="center" cellpadding="0" cellspacing="0">
  ...
  <tr>
      ...
      <tr>
        <td height="288" align="left" valign="top"><table width="630" align="left" cellpadding="0"  cellspacing="0">
          ...
          <tr>
			...
			if($_GET['tab']=="approvedondisplay"||$_GET['tab']=="") { ?>              
              <!--On Display-->
              <form name="ProductListForm1" method="post" action="manageMul.php?tab=approvedondisplay">
			  <script>
                ...
                <div id="tab">
                  <ul>
                    <li class="current"><a href="manageMul.php?tab=approvedondisplay" title="Display Product Item Online" class="content">On Display (<?php echo $total_testimonial_on_display; ?>)</a></li>
                    <li class=""><a href="manageMul.php?tab=approvednotondisplay" title="Not Display Product Item Online" class="content">Not On Display (<?php echo $total_testimonial_not_on_display; ?>)</a></li>
                  </ul>
                </div>
                ...
                <table border="0" cellpadding="4" cellspacing="1" width="100%">
                  <tr>
                    ...
                    <th width="15%" class="content">On Display</th>
					 <th width="26%" class="content">Uploaded Files</th>
                  ...
                  <tr>
                    <th width="5%" class="content">
                      ...
                      <input name="productIdListUpdate[]" type="hidden" value="<?php echo $row_Rs1['file_id'];?>">
                        <br />
                    </th>
                    ...
                    <?php echo $row_Rs1['subject']; ?><br>
                    <?php echo $row_Rs1['title']; ?>
                    </td>
					<td class="content">
	...
	$cat_row=mysqli_fetch_assoc($cat_Rs1);
					do{
	$check_cat01_q=mysqli_query($conn, "SELECT * FROM file_upload02 WHERE category like '%|".$cat_row['maincat_id']."|%' and category!='' and file_id='".$row_Rs1['file_id']."' ");
	$check_cat01=mysqli_num_rows($check_cat01_q);
				...
				?>
                          ...
                          <input type="checkbox" name="<?php echo $cat_row['maincat_id'].'category'.$row_Rs1['file_id'];?>" value="<?php echo $cat_row['maincat_id'];?>" <?php if($check_cat01>='1'){?> checked<?php }?>><?php echo $cat_row['maincat_name']; ?>
                            </div>
...
do{if(strlen($next_level2['position'])==$next_position_len2){ 
	...
	$check_cat02_q=mysqli_query($conn, "SELECT * FROM file_upload02 WHERE category like '%|".$next_level2['maincat_id']."|%' and category!='' and file_id='".$row_Rs1['file_id']."' ");
...
?>
          ...
          <input type="checkbox" name="<?php echo $next_level2['maincat_id'].'category'.$row_Rs1['file_id'];?>" value="<?php echo $next_level2['maincat_id'];?>" <?php if($check_cat02>='1'){?> checked<?php }?>><?php echo $next_level2['maincat_name']; ?></div>
...
do{if(strlen($next_level3['position'])>=$next_position_len3){ 
	$check_cat03_q = mysqli_query($conn, "SELECT * FROM file_upload02 WHERE category like '%|".$next_level3['maincat_id']."|%' and category!='' and file_id='".$row_Rs1['file_id']."' ");
...
?>
          ...
          <input type="checkbox" name="<?php echo $next_level3['maincat_id'].'category'.$row_Rs1['file_id'];?>" value="<?php echo $next_level3['maincat_id'];?>" <?php if($check_cat03>='1'){?> checked<?php }?>><?php echo $next_level3['maincat_name']; ?></div>
...
?>                       
					...
					<td class="content" align="center"><a href="editMul.php?file=<?php echo $row_Rs1['file_id']?>"><img src="../images/b_edit.png" width="16" height="15" border="0"></a>
<!--<input type="submit" name="<?php //echo 'changes'.$row_Rs1['file_id'];?>" value="Save Changes">--></td>
                  </tr>
                  <?php
						 } while($row_Rs1 = mysqli_fetch_assoc($Rs1));
					}
				...
				<tr><td colspan="5">
					  ...
					  <tr>
						<td><span class="content"><strong><a href="manageMul.php?tab=<?php echo $_GET['tab']?>&alphabet=A">A</a></strong></span></td>
						<td><span class="content"><strong><a href="manageMul.php?tab=<?php echo $_GET['tab']?>&alphabet=B">B</a></strong></span></td>
						<td><span class="content"><strong><a href="manageMul.php?tab=<?php echo $_GET['tab']?>&alphabet=C">C</a></strong></span></td>
						<td><span class="content"><strong><a href="manageMul.php?tab=<?php echo $_GET['tab']?>&alphabet=D">D</a></strong></span></td>
						<td><span class="content"><strong><a href="manageMul.php?tab=<?php echo $_GET['tab']?>&alphabet=E">E</a></strong></span></td>
						<td><span class="content"><strong><a href="manageMul.php?tab=<?php echo $_GET['tab']?>&alphabet=F">F</a></strong></span></td>
						<td><span class="content"><strong><a href="manageMul.php?tab=<?php echo $_GET['tab']?>&alphabet=G">G</a></strong></span></td>
						<td><span class="content"><strong><a href="manageMul.php?tab=<?php echo $_GET['tab']?>&alphabet=H">H</a></strong></span></td>
						<td><span class="content"><strong><a href="manageMul.php?tab=<?php echo $_GET['tab']?>&alphabet=I">I</a></strong></span></td>
						<td><span class="content"><strong><a href="manageMul.php?tab=<?php echo $_GET['tab']?>&alphabet=J">J</a></strong></span></td>
						<td><span class="content"><strong><a href="manageMul.php?tab=<?php echo $_GET['tab']?>&alphabet=K">K</a></strong></span></td>
						<td><span class="content"><strong><a href="manageMul.php?tab=<?php echo $_GET['tab']?>&alphabet=L">L</a></strong></span></td>
						<td><span class="content"><strong><a href="manageMul.php?tab=<?php echo $_GET['tab']?>&alphabet=M">M</a></strong></span></td>
						<td><span class="content"><strong><a href="manageMul.php?tab=<?php echo $_GET['tab']?>&alphabet=N">N</a></strong></span></td>
						<td><span class="content"><strong><a href="manageMul.php?tab=<?php echo $_GET['tab']?>&alphabet=O">O</a></strong></span></td>
						<td><span class="content"><strong><a href="manageMul.php?tab=<?php echo $_GET['tab']?>&alphabet=P">P</a></strong></span></td>
						<td><span class="content"><strong><a href="manageMul.php?tab=<?php echo $_GET['tab']?>&alphabet=Q">Q</a></strong></span></td>
						<td><span class="content"><strong><a href="manageMul.php?tab=<?php echo $_GET['tab']?>&alphabet=R">R</a></strong></span></td>
						<td><span class="content"><strong><a href="manageMul.php?tab=<?php echo $_GET['tab']?>&alphabet=S">S</a></strong></span></td>
						<td><span class="content"><strong><a href="manageMul.php?tab=<?php echo $_GET['tab']?>&alphabet=T">T</a></strong></span></td>
						<td><span class="content"><strong><a href="manageMul.php?tab=<?php echo $_GET['tab']?>&alphabet=U">U</a></strong></span></td>
						<td><span class="content"><strong><a href="manageMul.php?tab=<?php echo $_GET['tab']?>&alphabet=V">V</a></strong></span></td>
						<td><span class="content"><strong><a href="manageMul.php?tab=<?php echo $_GET['tab']?>&alphabet=W">W</a></strong></span></td>
						<td><span class="content"><strong><a href="manageMul.php?tab=<?php echo $_GET['tab']?>&alphabet=X">X</a></strong></span></td>
						<td><span class="content"><strong><a href="manageMul.php?tab=<?php echo $_GET['tab']?>&alphabet=Y">Y</a></strong></span></td>
						<td><span class="content"><strong><a href="manageMul.php?tab=<?php echo $_GET['tab']?>&alphabet=Z">Z</a></strong></span></td>
					  </tr>
					</table>
				  </td></tr>				  				  
                </table>
              ...
              <form name="ProductListForm2" method="post" action="manageMul.php?tab=approvednotondisplay">
			  <script>
                ...
                <div id="tab">
                  <ul>
                    <li class=""><a href="manageMul.php?tab=approvedondisplay" title="Display Product Item Online" class="content">On Display (<?php echo $total_testimonial_on_display; ?>)</a></li>
                    <li class="current"><a href="manageMul.php?tab=approvednotondisplay" title="Not Display Product Item Online" class="content">Not On Display (<?php echo $total_testimonial_not_on_display; ?>)</a></li>
                  </ul>
                </div>
				...
				?>
                  <tr>
					...
					<td class="content" align="center" style="padding:25px;">
                    <?php echo $row_Rs1['subject']; ?>
                    <br>
                    <?php echo $row_Rs1['title']; ?></td>
					<td class="content">
	...
	$cat_row=mysqli_fetch_assoc($cat_Rs1);
					do{
	$check_cat01_q=mysqli_query($conn, "SELECT * FROM file_upload WHERE category like '%|".$cat_row['maincat_id']."|%' and category!='' and file_id='".$row_Rs1['file_id']."' ");
	$check_cat01=mysqli_num_rows($check_cat01_q);
...
do{if(strlen($next_level2['position'])==$next_position_len2){ 
	...
	$check_cat02_q=mysqli_query($conn, "SELECT * FROM file_upload WHERE category like '%|".$next_level2['maincat_id']."|%' and category!='' and file_id='".$row_Rs1['file_id']."' ");
	$check_cat02=mysqli_num_rows($check_cat02_q);
...
do{if(strlen($next_level3['position'])>=$next_position_len3){ 
	$check_cat03_q = mysqli_query($conn, "SELECT * FROM file_upload WHERE category like '%|".$next_level3['maincat_id']."|%' and category!='' and file_id='".$row_Rs1['file_id']."' ");
	$check_cat03 = mysqli_num_rows($check_cat03_q);
?>
          <div style="float:left; width:80%; margin-left:50px;">
...
?>                       
                    ...
                    <td class="content" align="center"><a href="editMul.php?file=<?php echo $row_Rs1['file_id']?>"><img src="../images/b_edit.png" width="16" height="15" border="0"></a></td>
                  </tr>
					<?php
					} while($row_Rs1 = mysqli_fetch_assoc($Rs1));
					}
