...
//*************************************************Edit Country*************************************************
		$temp_query_Recordset = "SELECT * FROM album";					 
		...
		do
		{
			...
			{	
				...
				$insertSQL1 = "UPDATE album SET album_name='".$defenders->escapeInjection($_POST['album_name'.$row_Rs1['album_id']])."', 
	...
	$album_query=mysqli_query($conn, "SELECT album_name FROM album WHERE status!=-1 ORDER BY album_name");
	$album_set=mysqli_fetch_assoc($album_query);

	$selected_section_query=mysqli_query($conn, "SELECT * FROM album WHERE album_id='".$defenders->escapeInjection($_GET['id'])."' ");
...
//******************************************Add Country*************************************************
	...
	if($_POST['submit']==' Submit ')
	{
		...
		if(mysqli_query($conn, "INSERT INTO album (album_id, album_name, status, date_posted) VALUES ('', '$album', 1, '".date('Y-m-d')."')"))		
...
//*************************************************Manage Country*************************************************
	...
	///////////////display//////////////////////////////////////////////////////////////
	$temp_query_Recordset2 = "SELECT * FROM album WHERE status=0";					 
	...
	$temp_query_Recordset1 = "SELECT * FROM album WHERE status=1";					 
	...
	if(!empty($_POST['productIdList']))
	{
		...
		{
			$del_data="UPDATE album SET status='-1' WHERE album_id='$items_delete'";
			mysqli_select_db($conn, $database_pamconnection);
...
{
		$query="SELECT * FROM album WHERE status=1 ORDER BY position ASC, album_name ASC";
}
else if($_GET['tab']=="approvednotondisplay")
{
		$query="SELECT * FROM album WHERE status=0 ORDER BY position ASC, album_name ASC";
}
	...
	$query2=mysqli_query($conn, "SELECT * FROM album WHERE status=0");
	$total_testimonial_not_on_display = mysqli_num_rows($query2);
	$query=mysqli_query($conn, "SELECT * FROM album WHERE status=1");
...
function check()
{
	...
	{
		alert("Please enter title for the album.");
  ...
  <tr>
    <td align="left" valign="top">
      <table width="100%" align="left" cellpadding="2"  cellspacing="2" border="0">
        <form name="form1" method="post" action="createAlbum.php?add=1" enctype="multipart/form-data">
          <tr>
            <td colspan="2" height="32" class="main_title" align="center" bgcolor="#CCCCCC" valign="middle">CREATE ALBUM</td>
          </tr>
          <tr>
          ...
          <tr>
            <th width="35%" align="right" bgcolor="#EFEFEF" class="main_title"><span class="red">* </span>Add Album</th>
  ...
  <tr>
    <td>
      <table width="630" border="0" cellspacing="0" cellpadding="0">
        <tr>
          <td height="32" align="center" valign="middle" bgcolor="#CCCCCC"><span class="main_title">MANAGE ALBUM</span></td>
        </tr>
        <tr>
          <td height="288" align="left" valign="top"><table width="630" align="left" cellpadding="0"  cellspacing="0">
			...
			if($_GET['tab']=="approvedondisplay"||$_GET['tab']=="") { ?>
                    <!--On Display-->
                    <form name="form2" method="post" action="createAlbum.php?tab=approvedondisplay">
                      <script>
			  	function deleteProduct1()
				{
					...
					else
					{
                      ...
                      <div id="tab">
                        <ul>
                          <li class="current"><a href="createAlbum.php?tab=approvedondisplay" title="Display Product Item Online" class="content">On Display (
                                <?php if ($total_testimonial_on_display!=0){ echo $total_testimonial_on_display;}else{?>
                            0
                            <?php }?>
                            )</a></li>
                          <li class=""><a href="createAlbum.php?tab=approvednotondisplay" title="Not Display Product Item Online" class="content">Not On Display (
                      ...
                      <table border="0" cellpadding="4" cellspacing="1" width="100%">
                        <tr bgcolor="#CCCCCC">
                          ...
                          <th width="38%" class="content">Album</th>
				...
				?>
                    ...
                    <form name="ProductListForm2" method="post" action="createAlbum.php?tab=approvednotondisplay">
                      <script>
			  	function deleteProduct2()
				{
					...
					else
					{
                      ...
                      <div id="tab">
                        <ul>
                          <li class=""><a href="createAlbum.php?tab=approvedondisplay" title="Display Product Item Online" class="content">On Display(
                                <?php if ($total_testimonial_on_display!=0){ echo $total_testimonial_on_display;}else{?>
                            0
                            <?php }?>
                            )</a></li>
                          <li class="current"><a href="createAlbum.php?tab=approvednotondisplay" title="Not Display Product Item Online" class="content">Not On Display (
                                <?php if ($total_testimonial_not_on_display!=0){echo $total_testimonial_not_on_display;}else{?>
                            0
                            <?php }?>
                            )</a></li>
                      ...
                      <table border="0" cellpadding="4" cellspacing="1" width="100%">
                        <tr bgcolor="#CCCCCC">
                          <th width="5%" class="content">
                            <input name="Input" type="checkbox" value="" onClick="chkAll(this.form, 'productIdList[]', this.checked)">                          </th>
                          <th width="16%" class="content">On Display</th>
                          <th width="39%" class="content"> Album</th>
                            <th width="14%" class="content" align="center">Position No. </th>
                          <th width="26%" class="content" align="center">Edit</th>
                        </tr>
                        <tr>
...
<tr>
  <td valign="bottom" class="style3" colspan="2">
	  <table width="100%"  border="0" class="greybg">
		<tr>
		  <td height="32"><div align="center" class="style3">&copy;Copyright of WebAndYou<br>
...
<div id="Layer1" style="position:absolute; width:168px; height:24px; z-index:1; left: 740px; top: 4px;">
