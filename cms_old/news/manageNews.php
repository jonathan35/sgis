<?php require_once('../../Connections/pamconnection.php'); 
	...
	if($_POST['update']=='Save & Update'){	
		...
		if(!empty($_POST['productIdListUpdate']))
		{
			...
			{	 
				$category_check_query = mysqli_query($conn, "SELECT * FROM sections_categories WHERE status=1");
				...
				if($data_check!=''){
					$category_list_query = mysqli_query($conn, "SELECT * FROM sections_categories WHERE status=1");
...
if($_GET['sub']!=''){
	...
	$get_scat_query = " AND category like '%|".$get_scat."|%' ";
}
...
if($_GET['tab']=="approvedondisplay" || $_GET['tab']=="")
{
	if(!empty($_GET['alphabet'])) {
	 $colname1_Rs1=(get_magic_quotes_gpc()) ? $_GET['alphabet'] : addslashes($_GET['alphabet']);
	 	$query="SELECT * FROM events_02 WHERE status='1' AND name like '$colname1_Rs1%' $get_scat_query ";
	 }
	 else
		$query="SELECT * FROM events_02 WHERE status='1' $get_scat_query  ORDER BY date DESC";
}
elseif($_GET['tab']=="approvednotondisplay")
{
	if(!empty($_GET['alphabet'])) {
	 $colname1_Rs1=(get_magic_quotes_gpc()) ? $_GET['alphabet'] : addslashes($_GET['alphabet']);
	 $query="SELECT * FROM events_02 WHERE status='0' AND name like '$colname1_Rs1%' $get_scat_query ";
	 }
	 else	
		$query="SELECT * FROM events_02 WHERE status='0' $get_scat_query  ORDER BY date DESC";
}
	...
	$query2=mysqli_query($conn, "SELECT * FROM events_02 WHERE status=0 $get_scat_query ");
	$total_testimonial_not_on_display = mysqli_num_rows($query2);
	$query=mysqli_query($conn, "SELECT * FROM events_02 WHERE status=1 $get_scat_query ");
...
<?php $qry=mysqli_query($conn, "select * from sections_categories where status=1 $filter_section_limit_query order by position asc"); 
                ...
                <tr>
                  <td >
                    <table width="100%" border="0" cellspacing="0" cellpadding="0">
                        <tr>
                            <td align="right">
                                <?php include ("../sections_category_filter.php");?>                               
                            </td>
                        </tr>
                    </table>        
                  </td>
                ...
                <table border="0" cellpadding="4" cellspacing="1" width="100%">
                  <tr>
                    ...
                    <th width="15%" class="content">On Display</th>
					 <th width="26%" class="content"><?php echo $current_template_set['maincat_name']?></th>
                  ...
                  <tr>
					...
					<td class="content">
					 ...
					 <?php 
	$cat_Rs1=mysqli_query($conn, "SELECT * FROM sections_categories WHERE status=1 and section_type=2 and length(position)=1 $section_limit_query ORDER BY position ASC, maincat_name ASC");
...
<?
		...
		$next_level_query2 = mysqli_query($conn, "SELECT * FROM sections_categories WHERE status=1 and position like '".$previous_code2."%' ORDER BY position ASC, maincat_name ASC");
...
<?
		...
		$next_level_query3 = mysqli_query($conn, "SELECT * FROM sections_categories WHERE status=1 and position like '".$previous_code3."%' ORDER BY position ASC, maincat_name ASC");
			  ...
			  <script>
                ...
                <table border="0" cellpadding="4" cellspacing="1" width="100%">
                  ...
                  <tr>
                    ...
                    <!--onClick="return displayProduct();"-->
                    <td width="15%" class="content">
                      <input type="submit" name="<?php echo $row_Rs1['id']; ?>" value="Display" title="Not Display This Product At Your Online List"/>                      </td>
					<td class="content" align="center">
                    <?php echo $row_Rs1['name']; ?><br>
                    <img src="<?php echo "../../".$row_Rs1['image1']; ?>" width="133" height="100" border="0"></td>
					<td class="content">
