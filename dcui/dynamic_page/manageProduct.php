...
?>
                          ...
                          <input type="checkbox" value="<?php echo $rotcat['maincat_id']; ?>" name="<?php echo $rotcat['maincat_id'].'category'.$row_Rs1['id'];?>" <?php if($check_level1>='1'){?> checked<?php }?>>&nbsp;
                            ...
                            <?php echo $rotcat['maincat_name']; ?><?php if($rotcat['section_type']=='2') echo " <span class=\"red\">S</span>"; else echo " <span class=\"red\">D</span>";?> 
                           </div>
...
?>
   ...
   <?php }while($rotcat = mysqli_fetch_assoc($root_Rs1));?>
                    ...
                    <td class="content" align="center"><a href="editProduct.php?id=<?php echo $row_Rs1['id']?>"><img src="../images/b_edit.png" width="16" height="15" border="0"></a></td>
                  </tr>
                  <?php $count++;
...
<input type="submit" name="update" value="Save & Sort" onClick="return saveProduct();" title="Save & Sort item(s) position" style="width:90px; color:#333;">
				...
				?>
                  <tr bgcolor="<?php echo $bgcolor;?>">
                    <th width="9%" class="content" bgcolor="<?php echo $bgcolor;?>">
                      ...
                      <td class="content"><?=$row_Rs1['product_name']; ?></td>
                     <td class="content">
	...
	$count_sec = 0;
					do{
	$sdsdfsadf=mysqli_query($conn, "SELECT * FROM mydf_product WHERE maincat_id like '%|".$category_set['maincat_id']."|%' and maincat_id!='' and id='".$row_Rs1['id']."' ");
	$ggdsdf=mysqli_num_rows($sdsdfsadf);
						$count_sec++;
						//if($count_sec==1){
						?>

                            
                            <div style="position:relative; left:-2px; height:10px;">
                             <input type="checkbox" name="<?php echo $category_set['maincat_id'].'category'.$row_Rs1['id'];?>" value="<?php echo $category_set['maincat_id'];?>" <?php if($ggdsdf>='1'){?> checked<?php }?>>
                            </div>
                             <div style="position:relative; left:19px; top:-8px; height:10px;" class="content_small">
                            <?php echo $category_set['maincat_name'];?>
                            </div>
<?
	$category_query02=mysqli_query($conn, "SELECT * FROM murum_section WHERE status=1 and length(position)=2 and maincat_id='".$category_set['maincat_id']."' ORDER BY position ASC, maincat_name ASC");
	$category_set02=mysqli_fetch_assoc($category_query02);
	
	if($category_set02!=''){
							do{
	$sdsdfsadf02=mysqli_query($conn, "SELECT * FROM mydf_product WHERE category_id like '%|".$category_set02['maincat_id']."|%' and category_id!='' and id='".$row_Rs1['id']."' ");
	$ggdsdf02=mysqli_num_rows($sdsdfsadf02);

?>
							<div style="position:relative; margin:0 0 0 20px;">
                            <div style="position:relative; left:-2px; height:10px;">
                             <input type="checkbox" name="<?php echo $category_set02['maincat_id'].'subcat'.$row_Rs1['id'];?>" value="<?php echo $category_set02['maincat_id'];?>" <?php if($ggdsdf02>='1'){?> checked<?php }?>>
                            </div>
                             <div style="position:relative; left:19px; top:-10px; height:10px;" class="content_small">
                            <?php echo $category_set02['maincat_name'];?>
                            </div>  
                            </div>                          
                    		<?php }while($category_set02=mysqli_fetch_assoc($category_query02));}?>
                            
                            <!--</td>
                        <?php //if($count_sec==3){$count_sec = 0;?>
                        </tr> --><?php //}?>
