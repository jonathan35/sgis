<?php require_once('../../Connections/pamconnection.php'); 
	...
	do
	{
		if($_POST['save'.$row_Rs1['id']]=="Save")
			mysqli_query($conn, "update washington_gallery set position='".$_POST['position'.$row_Rs1['id']]."' WHERE id='".$row_Rs1['id']."'"); 
...
if($_GET['tab']=="approvedondisplay"||$_GET['tab']=="")
{
	if(!empty($_GET['alphabet'])) {
	 ...
	 $query="SELECT * FROM washington_gallery WHERE status=1 AND product_name like '$colname1_Rs1%' ORDER BY album_id desc, position asc";
		...
		$query="SELECT * FROM washington_gallery WHERE status=1 ORDER BY album_id desc, position asc";
}
...
<body>
  <div id="Layer1" style="position:absolute; width:200px; height:36px; z-index:1; left: 696px; top: 4px;">
			  ...
			  <script>
				...
				$bgcolor='#FFFFFF';
                  ...
                  <tr bgcolor="<?php echo $bgcolor;?>">
                    ...
                    <td class="content" align="center"><a href="editPhoto.php?id=<?php echo $row_Rs1['id']?>"><img src="../images/b_edit.png" width="16" height="15" border="0"></a></td>
                  </tr>
                  <?php $count++;
						 } while($row_Rs1 = mysqli_fetch_assoc($Rs1));
					}
