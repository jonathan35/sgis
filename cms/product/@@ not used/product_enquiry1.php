...
if($_POST['delete']=='Delete'){
	...
	if(!empty($_POST['productIdList']))
	{
		...
		{
				mysqli_query($conn, "DELETE FROM inquiry_log1 where id='".$items_delete."'");
		}
	}
}
...
$query = mysqli_query($conn, "SELECT * FROM inquiry_log1 WHERE id='".$_GET['id']."'");
...
<html>
<head>
<title>Weda Choice Feedback form</title>
...
mysqli_query($conn, "UPDATE inquiry_log1 SET reply_status='1', date_posted_reply='".date("Y-m-d")."', time_reply='".date("g:i a")."' WHERE id='".$_GET['id']."' ");
...
if($_GET['send']=='true')
{
	...
	if(mail($mailto, $subject, $messageproper,"MIME-Version: 1.0\n"."Content-type: text/html; charset=iso-8859-1"))
	   {
	   		mysqli_query($conn, "UPDATE inquiry_log1 SET status=1, date_posted_reply='".date_posted("Y-m-d")."', time_reply='".date_posted("g:i:s")."', adminmessage='".$_POST['message']."', quoteprice='".$_POST['price']."' WHERE id='".$_GET['id']."' ");
			$send='success'; 
	   }
	 else 
	   {
...
if($_GET['read']=='yes')
{
	mysqli_query($conn, "UPDATE inquiry_log1 SET readunread='1' WHERE id='".$_GET['id']."' ");
}
	
if($_GET['st']==-1)
$query = "SELECT * FROM inquiry_log1 WHERE status=0 ORDER BY date_posted DESC";
else if($_GET['st']==1)
$query = "SELECT * FROM inquiry_log1 WHERE status=1 ORDER BY date_posted ASC";
else
$query = "SELECT * FROM inquiry_log1 ORDER BY name ASC";
...
if (!empty($_SERVER['QUERY_STRING'])) {
  ...
  if (count($newParams) != 0) {
	$queryString_Rs1 = "&" . htmlentities(implode("&", $newParams));
				...
				$check=mysqli_query($conn, "SELECT id FROM inquiry_log1 WHERE status=1");
				$total=mysqli_num_rows($check);

				$check2=mysqli_query($conn, "SELECT id FROM inquiry_log1 WHERE status=0");
...
<table width="630" border="1" cellspacing="0" cellpadding="0">
  <tr>
    ...
    <?php }?>                    
              ...
              <form name="ProductListForm2" method="post" action="product_enquiry1.php">
                <script>
...
<div class="content"> Select
				  ...
				  ?>
                    ...
                    <td width="4%" class="content"><input type="checkbox" value="<?php echo $row_Rs1['id']; ?>" name="productIdList[]">                     </td>
                      <td align="center">
                        ...
                        <a href="product_enquiry1.php?st=<?php echo $_GET['st'];?>&read=yes&id=<?php echo $row_Rs1['id']?>"><img src="../images/unread.gif" width="25" height="20" border="0"></a>
                        <?php }if($row_Rs1['readunread']==1){?>
                        <a href="product_enquiry1.php?st=<?php echo $_GET['st'];?>&read=yes&id=<?php echo $row_Rs1['id']?>"><img src="../images/read.gif" width="27" height="20" border="0"></a>
                        <?php }?>
                      </td>
		...
		{
		$record_query=mysqli_query($conn, "SELECT * FROM inquiry_log1 WHERE id='".$_GET['id']."' ");
...
<table width="100%" border="0" cellpadding="4" cellspacing="6">
      <tr>
        <td class="content"><?php include("customer_info1.php");?></td>
      </tr>
      
      <tr>
        <td class="content"><?php include("reply1.php");?></td>
      </tr>      
