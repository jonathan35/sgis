<?php require_once('../Connections/pamconnection.php'); 
	...
	if(!empty($_POST['submit'])){
		$message = $fileManager->uploadFile($_FILES['file']['tmp_name'], $_FILES['file']['type']);
	}
...
<div>
    ...
    <div>
        ...
        <form name="form" action="new_file.php?type=<?=$_GET['type']?>" method="post" enctype="multipart/form-data">
            ...
            <div style="font-size:11px; padding:8px 8px 0 66px; color:#0099FF;">
                - Max. file size 5MB only.<br />
			   <?php if($_GET['type'] == 'photo' || $_GET['type'] == 'image'){ ?>
               		- Allowed file format: jpeg, gif, png only.<br />
               <?php }elseif($_GET['type'] == 'attachment' || $_GET['type'] == 'pdf'){?> 
                	- Allowed file format: PDF only.<br />
               <?php }?>
            </div>
        </form>
    </div>
        ...
        if($files){		
            foreach($files as $id => $file){
              ...
              <div class="file <?=$classType?>">
                ...
                <div id="<?=$divID?>" onClick="selectText('<?=$divID?>')" class="path"><?php 
					if($_GET['type'] == 'photo' || $_GET['type'] == 'image'){
						echo str_replace('../','../../',$file['path']);
					}elseif($_GET['type'] == 'attachment' || $_GET['type'] == 'pdf'){
						echo str_replace('../','',$file['path']);
					}?></div>
              </div>                
        <?php }
