	...
	if($_GET['maincat']!=''){
		if($_GET['subcat']!=''){
			if(!empty($_GET['alphabet'])) {
				 ...
				 $query="SELECT * FROM events_02 WHERE status=1 AND name like '$colname1_Rs1%' and category='".$_GET['maincat']."' and brand='".$_GET['subcat']."' ";
			}else
				$query="SELECT * FROM events_02 WHERE status=1 and category='".$_GET['maincat']."' and brand='".$_GET['subcat']."' ORDER BY name ASC";
		}elseif(!empty($_GET['alphabet'])) {
			 $colname1_Rs1=(get_magic_quotes_gpc()) ? $_GET['alphabet'] : addslashes($_GET['alphabet']);
			 $query="SELECT * FROM events_02 WHERE status=1 AND name like '$colname1_Rs1%' and category='".$_GET['maincat']."' ";
		}else
			$query="SELECT * FROM events_02 WHERE status=1 and category='".$_GET['maincat']."' ORDER BY name ASC";			
	}elseif($_GET['subcat']!=''){
		if(!empty($_GET['alphabet'])) {
			 $colname1_Rs1=(get_magic_quotes_gpc()) ? $_GET['alphabet'] : addslashes($_GET['alphabet']);
			 $query="SELECT * FROM events_02 WHERE status=1 AND name like '$colname1_Rs1%' and brand='".$_GET['subcat']."' ";
		}else
			$query="SELECT * FROM events_02 WHERE status=1 and brand='".$_GET['subcat']."' ORDER BY name ASC";
	}
	elseif(!empty($_GET['alphabet'])) {
	 $colname1_Rs1=(get_magic_quotes_gpc()) ? $_GET['alphabet'] : addslashes($_GET['alphabet']);
	 $query="SELECT * FROM events_02 WHERE status=1 AND name like '$colname1_Rs1%'";
	 }
	 else
		$query="SELECT * FROM events_02 WHERE status=1 ORDER BY name ASC";
}
else if($_GET['tab']=="approvednotondisplay")
{
	if($_GET['maincat']!=''){
		if($_GET['subcat']!=''){
			if(!empty($_GET['alphabet'])) {
				 $colname1_Rs1=(get_magic_quotes_gpc()) ? $_GET['alphabet'] : addslashes($_GET['alphabet']);
				 $query="SELECT * FROM events_02 WHERE status=0 AND name like '$colname1_Rs1%' and category='".$_GET['maincat']."' and brand='".$_GET['subcat']."' ";
			}else
				$query="SELECT * FROM events_02 WHERE status=0 and category='".$_GET['maincat']."' and brand='".$_GET['subcat']."' ORDER BY name ASC";
		}elseif(!empty($_GET['alphabet'])) {
			 $colname1_Rs1=(get_magic_quotes_gpc()) ? $_GET['alphabet'] : addslashes($_GET['alphabet']);
			 $query="SELECT * FROM events_02 WHERE status=0 AND name like '$colname1_Rs1%' and category='".$_GET['maincat']."' ";
		}else
			$query="SELECT * FROM events_02 WHERE status=0 and category='".$_GET['maincat']."' ORDER BY name ASC";			
	}elseif($_GET['subcat']!=''){
		if(!empty($_GET['alphabet'])) {
			 $colname1_Rs1=(get_magic_quotes_gpc()) ? $_GET['alphabet'] : addslashes($_GET['alphabet']);
			 $query="SELECT * FROM events_02 WHERE status=0 AND name like '$colname1_Rs1%' and brand='".$_GET['subcat']."' ";
		}else
			$query="SELECT * FROM events_02 WHERE status=0 and brand='".$_GET['subcat']."' ORDER BY name ASC";
	}
	elseif(!empty($_GET['alphabet'])) {
	 $colname1_Rs1=(get_magic_quotes_gpc()) ? $_GET['alphabet'] : addslashes($_GET['alphabet']);
	 $query="SELECT * FROM events_02 WHERE status=0 AND name like '$colname1_Rs1%'";
	 }
	 else	
		$query="SELECT * FROM events_02 WHERE status=0 ORDER BY name ASC";
}
	...
	$query2=mysqli_query($conn, "SELECT * FROM events_02 WHERE status=0");
	$total_testimonial_not_on_display = mysqli_num_rows($query2);
	$query=mysqli_query($conn, "SELECT * FROM events_02 WHERE status=1");
          ...
          <tr>
            <td width="636" height="244" align="left" valign="top"><div align="left">
              ...
              <table width="100%" border="0" cellpadding="2" cellspacing="2" class="content" id="data">
                <tr>
                  <td width="52%" align="left"><a href="manageNews.php?tab=approvedondisplay">Approved - On Display</a></td>
                  <td width="48%"><?php echo $total_testimonial_on_display; ?></td>
                </tr>
                <tr>
                  <td align="left"><a href="manageNews.php?tab=approvednotondisplay">Approved - Not On Display</a></td>
                  <td><?php echo $total_testimonial_not_on_display; ?></td>
                </tr>
               <tr>
                  <td colspan="2" align="left">
					<table width="100%" border="0" align="left" cellpadding="2"  cellspacing="2">
						<form name="form1" method="post" action="manageNews.php" enctype="multipart/form-data" id="form">
						<tr>
               ...
               <?php if($row_Rs1=='' && $row_Rs2!='' && $row_Rs2['status']!=0){?>
               <tr>
                 <td colspan="2" align="left"><table width="100%" border="0" cellpadding="0" cellspacing="0">
                   <tr>
                       <td width="15%">Go To Backup:                   &nbsp;</td>
                       <td width="85%"><form name="form2" method="post" action="manageNews2.php">
                         <label>
                           <input type="submit" name="backup" id="button" value="Backup">
                         </label>
                       </form></td>
                       </tr>
                     </table></td>
               ...
               <tr>
                 <td colspan="2" align="left"><table width="100%" border="0" cellpadding="0" cellspacing="0">
                   <tr>
                     <td width="15%">Go To Backup</td>
                     <td width="85%"><form name="form2" method="post" action="manageNews3.php">
                         <label>
                           <input type="submit" name="backup" id="button" value="Backup">
                         </label>
                       </form></td>
			...
			if($_GET['tab']=="approvedondisplay"||$_GET['tab']=="") { ?>              
			  ...
			  <script>
                ...
                <div id="tab">
                  <ul>
                    <li class="current"><a href="manageNews.php?tab=approvedondisplay" title="Display Product Item Online" class="content">On Display</a></li>
                    <li class=""><a href="manageNews.php?tab=approvednotondisplay" title="Not Display Product Item Online" class="content">Not On Display</a></li>
                  </ul>
                </div>
...
<!--<input type="submit" name="<?php //echo 'changes'.$row_Rs1['id'];?>" value="Save Changes">--></td>
			  ...
			  <script>
                ...
                <div id="tab">
                  <ul>
                    <li class=""><a href="manageNews.php?tab=approvedondisplay" title="Display Product Item Online" class="content">On Display</a></li>
                    <li class="current"><a href="manageNews.php?tab=approvednotondisplay" title="Not Display Product Item Online" class="content">Not On Display</a></li>
                  </ul>
                </div>
				...
				?>
                  <tr>
                    ...
                    <td class="content" align="center"><a href="editNews.php?id=<?php echo $row_Rs1['id']?>"><img src="../images/b_edit.png" width="16" height="15" border="0"></a>
<!--<input type="submit" name="<?php //echo 'changes'.$row_Rs1['id'];?>" value="Save Changes">--></td>
                  </tr>
					<?php
					} while($row_Rs1 = mysqli_fetch_assoc($Rs1));
