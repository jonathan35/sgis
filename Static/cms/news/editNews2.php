...
session_start();
	...
	$query2=mysqli_query($conn, "select * from events_02_bk where id='$abc'");
	$set2=mysqli_fetch_assoc($query2);
	$query1=mysqli_query($conn, "select * from events_02 where id='".$set2.'$abc'."'");
...
/*
	if($_GET['wp']==1)
	mysqli_query($conn, "update events_02_bk set image1='' where id='$abc'");	
	if($_GET['wp']==1)
	mysqli_query($conn, "update events_02 set image1='' where id='$abc'");		
	if($_GET['cmd']=='pdf')
	mysqli_query($conn, "update events_02_bk set pdf_file='' where id='$abc'");
	if($_GET['cmd']=='pdf')
...
*/	
	...
	if($_FILES['photo1']!='')
		{
			...
			{ 
				...
				{
					//mysqli_query($conn, "update events_02_bk set image1='$file1' where id='$abc'");
				} 
				else 
				{		
		...
		if($_FILES['pdf_file']!='')
	{
		...
		{ 
			...
			{
				//mysqli_query($conn, "update events_02_bk set pdf_file='$pdf' where id='$abc'");
			}  
			else 
			{echo 'File could not be uploaded to server.<BR>';} 
			if ( move_uploaded_file( $pdf_source, $pdf ) ) 
	...
	//if(mysqli_query($conn, "update events_02_bk set name='".$_POST['name']."', weblink='".$_POST['weblink']."', description='$description', date='".$_POST['date']."' where id='$abc'"))
...
values ('".$set2['id']."', '".$set2['name']."', '".$set2['image1']."', '".$set2['pdf_file']."', '".$set2['weblink']."', '$newstype', '".$set2['description']."', '".$set2['date']."')"))
	...
	$save='<font color=#CC3300>Failed</font>';}
	//if($set!=''){
	//if(mysqli_query($conn, "update events_02 set name='".$_POST['name']."', weblink='".$_POST['weblink']."', description='$description', date='".$_POST['date']."' where id='$abc'"))
	//$save='<font color=#336600>Successful</font>';
	//else
	//$save='<font color=#CC3300>Failed</font>';}	
	
}
$selected_query=mysqli_query($conn, "select * from events_02_bk where id='$abc'");
...
<table width="100%" align="left" cellpadding="2"  cellspacing="3">
	<form name="form1" method="post" action="editNews2.php?id=<?php echo $_GET['id']?>" enctype="multipart/form-data">
	<tr>
	...
	<?php echo $save;?>
	  ...
	  <tr>
<th align="right" bgcolor="#EFEFEF" class="main_title" valign="top"><strong>Image : </strong><br>
          <span class="content"> (recommended image size<br>
          <span class="red">480 x 360 pixels </span> )</span></th>
          <td class="content"> Photo 1&nbsp;&nbsp;&nbsp;<?php if($selected['image1']!=''){?><a href="editNews2.php?<?php echo $_SERVER['QUERY_STRING']?>&wp=1"><img src="../images/b_drop.png" width="16" height="15" border="0"></a><?php }?><br>
				<?php if($selected['image1']!=''){$photo1=$selected['image1'];}else{$photo1='../images/no_photo.gif';}?>
                  <img name="thisImage1" id="thisImage1" src="<?php echo $photo1?>" width="80" height="60"><br>
	...
	<tr>
	  ...
	  <td><input name="pdf_file" type="file" id="pdf_file"><?php if($selected['pdf_file']!=''){?> 
		<a href="editNews2.php?id=<?php echo $_GET['id']?>&cmd=pdf">
	...
	<tr>
		...
		<td align="left">
		<textarea name="description" rows="15" cols="50"><?php echo $selected['description']?></textarea>
		</td>
	</tr>
	<tr>
